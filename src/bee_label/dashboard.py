...
import streamlit
from bee_label.enums import COLORS, JarSize
...
if picked is None:
    raise ValueError("Aucune étiquette sélectionnée")
...
if uploadedfile is None and picked in COLORS:
    honey_color = streamlit.color_picker(f"Je connais la couleur du miel de {picked}, mais tu peux la modifier", COLORS[picked])
else:
    honey_picked = streamlit.radio("Choisis une couleur de miel", COLORS)
    if honey_picked is None:
        raise ValueError("Aucune couleur de miel sélectionnée")

    honey_color = streamlit.color_picker("Si besoin tu peux modifier la couleur", COLORS[honey_picked])
...
try:
    ...
    name = streamlit.text_input("Nom du miel pour le nom de l'image", picked)
    if size is None:
            raise ValueError("Aucune taille de pot sélectionnée")
    ...
    streamlit.download_button(
        "Télécharger l'image",
        data=byte_io,
        file_name=f"{name} ({size.value}).png",
        mime="image/png",
    )
